...
function renderBookings(bookings, gridId , sort ) {
    ...
    try {
        ...
        const sortedBookings = sort == true ? bookings.sort((a, b) => {
                                                const timeA = timeToMinutes(a.time.substring(0, a.time.indexOf("s")));
                                                const timeB = timeToMinutes(b.time.substring(0, a.time.indexOf("s")));
                                                return timeA - timeB;
                                            })
                                            : [...bookings].reverse();
...
async function getSpecialSalonBookingData(salonId , salonName, options = {}) {
    try {
        ...
        if (error.name === 'AbortError') {
            console.log(`Fetch aborted for path: ${path}`);
            throw error;
        }
